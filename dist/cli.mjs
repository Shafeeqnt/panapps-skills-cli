...
import { a as M, c as Y, d as he, f as ve, g as require_picocolors, h as pD, i as Ie, l as be, m as ye, n as require_dist, o as Me, p as xe, r as esm_default, s as Se, t as xdgConfig, u as fe } from "./_chunks/libs/common.mjs";
...
async function searchMultiselect(options) {
	...
	return new Promise((resolve) => {
		...
		const keypressHandler = (_str, key) => {
			...
			const filtered = getFiltered();
			if (key.name === "return") {
				if (selected.size === 0 && filtered[cursor]) selected.add(filtered[cursor].value);
				submit();
				return;
			}
...
function parseFrontmatter(raw) {
	...
	return {
		data: (0, import_dist.parse)(match[1]) ?? {},
		content: match[2] ?? ""
	};
}
...
const AGENTS_DIR$2 = ".agent";
const SKILLS_SUBDIR = "skills";
const UNIVERSAL_SKILLS_DIR = ".agent/skills";
const home = homedir();
...
const agents = {
	...
	amp: {
		...
		globalSkillsDir: join(configHome, "agent/skills"),
		detectInstalled: async () => {
...
function getUniversalAgents() {
	return Object.entries(agents).filter(([_, config]) => config.skillsDir === UNIVERSAL_SKILLS_DIR && config.showInUniversalList !== false).map(([type]) => type);
}
function getNonUniversalAgents() {
	return Object.entries(agents).filter(([_, config]) => config.skillsDir !== UNIVERSAL_SKILLS_DIR || config.showInUniversalList === false).map(([type]) => type);
}
function isUniversalAgent(type) {
	return agents[type].skillsDir === UNIVERSAL_SKILLS_DIR;
}
...
async function installSkillForAgent(skill, agentType, options = {}) {
	...
	try {
		...
		if (agentDir === canonicalDir) return {
			success: true,
			path: agentDir,
			mode: "symlink",
...
async function selectAgentsInteractive(options) {
	...
	return await searchMultiselect({
		...
		items: [...universalChoices, ...otherChoices],
		initialSelected: []
	});
}
...
async function handleWellKnownSkills(source, url, options, spinner) {
	...
	if (targetAgents.length === 0) {
		M.warn("No agents selected. Skills will not be installed.");
		process.exit(0);
	}
	let installGlobally = options.global ?? false;
	const supportsGlobal = targetAgents.some((a) => agents[a] && agents[a].globalSkillsDir !== void 0);
	...
	if (successful.length > 0) {
		...
		for (const [skillName, skillResults] of bySkill) {
			...
			if (firstResult.mode === "copy") {
				...
				const shownPaths = /* @__PURE__ */ new Set();
	...
	try {
		...
		if (options.agent?.includes("*")) {
			...
			if (installedAgents.length === 0) if (options.yes) {
				...
				targetAgents = selected;
				if (targetAgents.length === 0) {
					M.warn("No agents selected. Please select at least one agent (e.g. Claude Code) by pressing Space before pressing Enter.");
					await cleanup(tempDir);
					process.exit(0);
				}
			}
		...
		for (const skill of selectedSkills) {
			...
			if (blobResult && "files" in skill) {
				...
				for (const file of blobSkill.files) if (file.contents.includes("{{FRAMEWORK}}") || file.contents.includes("{{FRAMEWORK_TITLE}}") || originalName.toLowerCase().includes("framework")) needsFrameworkPrompt = true;
			} else {
				const skillMdPath = join(skill.path, "SKILL.md");
				try {
					const content = await readFile(skillMdPath, "utf-8");
					if (content.includes("{{FRAMEWORK}}") || content.includes("{{FRAMEWORK_TITLE}}") || originalName.toLowerCase().includes("framework")) {
			...
			if (needsFrameworkPrompt) {
				...
				try {
					...
					frameworkOptions = JSON.parse(text).map((f) => ({
						value: f,
						label: f
					}));
					...
					M.warn("Gemini API Error: " + (e instanceof Error ? e.message : String(e)));
					frameworkOptions = [
						{
							value: "React",
							label: "React"
				...
				let frameworkChoice = await ve({
					message: `Which framework are you using for ${import_picocolors.default.cyan(originalName)}?`,
					options: frameworkOptions
				});
				if (pD(frameworkChoice)) {
					xe("Installation cancelled");
					await cleanup(tempDir);
					process.exit(0);
				}
				...
				if (frameworkChoice === "other") {
					const textChoice = await he({ message: "Type your framework name:" });
					if (pD(textChoice)) {
						xe("Installation cancelled");
						await cleanup(tempDir);
						process.exit(0);
					}
					frameworkRaw = textChoice.trim();
				} else frameworkRaw = frameworkChoice.trim();
				...
				const newSkillName = originalName.replace("framework", framework).replace("{{FRAMEWORK}}", framework);
				skill.name = newSkillName;
				const processContent = (content) => {
					let updated = content.replace(/\{\{FRAMEWORK\}\}/g, framework).replace(/\{\{FRAMEWORK_TITLE\}\}/g, frameworkTitle).replace(/\{\{SKILL_NAME\}\}/g, newSkillName);
					updated = updated.replace(new RegExp(`name:\\s*"?${originalName}"?`), `name: ${newSkillName}`);
					return updated;
				};
				if (blobResult && "files" in skill) {
					const blobSkill = skill;
					blobSkill.name = newSkillName;
					for (const file of blobSkill.files) file.contents = processContent(file.contents);
				} else for (const file of filesToUpdate) {
...
async function runInstallFromLock(args) {
	...
	if (remoteCount > 0) M.info(`Restoring ${import_picocolors.default.cyan(String(remoteCount))} skill${remoteCount !== 1 ? "s" : ""} from skills-lock.json into ${import_picocolors.default.dim(".agent/skills/")}`);
