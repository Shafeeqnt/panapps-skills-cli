	...
	try {
		...
		for (const skill of selectedSkills) {
			...
			if (blobResult && "files" in skill) {
				...
				for (const file of blobSkill.files) if (file.contents.includes("{{FRAMEWORK}}") || file.contents.includes("{{FRAMEWORK_TITLE}}")) needsFrameworkPrompt = true;
			} else {
			...
			if (needsFrameworkPrompt) {
				...
				try {
					...
					const data = await (await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyAAKMabpS5F487fzRL-EdOfSWSbsyhMAjI", {
						method: "POST",
